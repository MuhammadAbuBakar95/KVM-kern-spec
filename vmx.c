...
static int handle_exception(struct kvm_vcpu *vcpu)
{
	...
	if (is_invalid_opcode(intr_info)) {
	 ...
	 //    if(dt_handle_exception(vcpu, instp)) {
		// 	return 1;
		// }
...
bool dt_exception(struct kvm_vcpu *vcpu) {
	HAS_SWITCHED = FALSE;
	// dt_printk("checking if can handle");
	HAS_HANDLED = dt_handle_exception(vcpu, vmcs_readl(GUEST_RIP));
	// dt_printk("HAS_HANDLED is %d", HAS_HANDLED);
	return HAS_HANDLED;
}
...
static void __noclone vmx_vcpu_run(struct kvm_vcpu *vcpu)
{
	...
	// if(!initialized) {
	// 	initialized = true;
	// } else if(HAS_SWITCHED) {
	// 	num_switches++;
	// 	diff_t = get_time_from(start_t);
	// 	switching_exit_time += diff_t;
	// 	dt_printk("switching exit diff_t is %lld, switching exit time is %lld", diff_t, switching_exit_time);
	// 	dt_printk("num_switches in exit is %d", num_switches);
	// }
